...
好，到这里，你应该对一些操作Linux系统的命令有了一些初步的体会了，我们这里就挑选了几个最基本的命令进行介绍，在使用Linux系统的过程中，希望各位同学遇到不会的操作时候，主动利用**Google**等搜索引擎查询使用方法，或者英语基础好一些的同学可以使用`man`命令查询用法，然后在这个过程中不断记忆和重复，最终达到操作Linux如行云流水一样。
